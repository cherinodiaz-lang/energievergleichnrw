...
export default function Header() {
  ...
  return (
    <>
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50 h-14 sm:h-16 lg:h-20 shadow-sm">
        <div className="max-w-[120rem] mx-auto px-4 sm:px-6 lg:px-12 h-full">
          <div className="flex items-center justify-between h-full gap-3 sm:gap-4">
            {/* Logo */}
            <Link
              to={ROUTES.home}
              className="flex items-center gap-2 sm:gap-2.5 group min-w-0 flex-shrink-0"
            ...
            >
              ...
              <div className="min-w-0 flex flex-col leading-tight">
                <span className="font-heading font-semibold text-xs sm:text-sm lg:text-base text-primary">
                  Energievergleich
                </span>
                <span className="font-heading text-xs sm:text-sm lg:text-base text-primary font-bold">
                  NRW
                </span>
              </div>
            </Link>
            ...
            <button
              ...
              className="lg:hidden p-2 text-gray-700 hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary rounded transition-colors min-h-12 min-w-12"
          ...
          {mobileMenuOpen && (
            <nav
              id="mobile-menu"
              className="lg:hidden py-3 sm:py-4 border-t border-gray-200 overflow-y-auto max-h-[calc(100vh-56px)] sm:max-h-[calc(100vh-64px)] lg:max-h-[calc(100vh-80px)]"
            ...
            >
              <ul className="flex flex-col divide-y divide-gray-200">
                {NAV_MAIN.map((item) => (
                  <li key={item.key}>
                    <>
                      {item.submenu ? (
                        <div className="flex items-center justify-between py-3 sm:py-4 px-4 sm:px-5 min-h-12">
                          <Link
                            to={item.to}
                        ...
                        <Link
                          to={item.to}
                          onClick={() => setMobileMenuOpen(false)}
                          className={`block w-full text-left font-paragraph font-medium text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-primary focus:ring-inset transition-colors py-3 sm:py-4 px-4 sm:px-5 min-h-12 ${
                            isActiveLink(item.to)
                          ...
                          {item.submenu.map((subitem) => (
                            <li key={subitem.key}>
                              <Link
                                to={subitem.to}
                                onClick={() => setMobileMenuOpen(false)}
                                className={`block font-paragraph text-sm font-medium transition-colors py-3 sm:py-4 px-6 sm:px-7 min-h-12 flex items-center ${
                                  isActiveLink(subitem.to)
              ...
              {/* Mobile CTA Button */}
              <div className="mt-3 sm:mt-4 px-4 sm:px-5 pt-3 sm:pt-4 border-t border-gray-200">
                <Button
                  onClick={handleCtaClick}
