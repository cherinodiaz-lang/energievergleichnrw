      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50 h-14 sm:h-16 lg:h-20">
        <div className="max-w-[120rem] mx-auto px-3 sm:px-6 lg:px-12 h-full">
          <div className="flex items-center justify-between h-full">
            ...
            {/* Desktop Navigation */}
            <nav className="hidden lg:flex flex-nowrap items-center gap-6 xl:gap-8" aria-label="Hauptnavigation">
              {NAV_MAIN.map((item) => (
                <div key={item.key} className="relative group">
                  <Link
                    to={item.to}
                    className={`font-paragraph text-sm xl:text-base font-medium focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded px-2 py-1 transition-colors flex items-center gap-1 min-h-12 whitespace-nowrap relative z-10 before:content-none after:content-none ${
