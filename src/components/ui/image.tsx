...
const getImageData = (url: string, imageProps: ImageProps): ImageData | undefined => {
  ...
  if (url.startsWith(wixImagePrefix)) {
    ...
    const originWidth =
      imageProps.originWidth || parseInt(searchParams.get('originWidth') || '0', 10);
    const originHeight =
      imageProps.originHeight || parseInt(searchParams.get('originHeight') || '0', 10);
...
const WixImage = forwardRef<HTMLImageElement, WixImageProps>(
  ({ data, fittingType = 'fill', ...imgProps }, parentRef) => {
    ...
    if (!size) {
      const { uri, ...placeholder } = getPlaceholder(fittingType ?? 'fit', data, {
        htmlTag: 'img',
      });
      ...
      return (
        <img
          ref={ref}
          src={`${STATIC_MEDIA_URL}${uri}`}
          style={placeholder.css.img}
          {...placeholder.attr}
          {...imgProps}
        />
      );
