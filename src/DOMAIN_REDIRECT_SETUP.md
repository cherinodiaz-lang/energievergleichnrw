# Domain Redirect Setup Guide
## energievergleich.shop as Primary Domain with energievergleich.shop Redirect

**Last Updated:** January 9, 2026  
**Status:** Ready for Implementation

---

## 🎯 Objective

Set **energievergleich.shop** as the primary domain and redirect **energievergleich.shop** to it with a 301 permanent redirect, ensuring only energievergleich.shop is indexed by search engines.

---

## ✅ Current Configuration Status

### Code-Level Configuration
- ✅ **SEO Config**: Primary domain set to `energievergleich.shop`
- ✅ **Canonical URLs**: All pages use `https://energievergleich.shop`
- ✅ **Robots.txt**: References `energievergleich.shop` only
- ✅ **Sitemap.xml**: All URLs point to `energievergleich.shop`
- ✅ **SEO Head Component**: Generates canonical tags for `energievergleich.shop`

### What Still Needs Configuration
- ⚠️ **Wix Domain Settings**: Set primary domain and configure redirect
- ⚠️ **Google Search Console**: Configure preferred domain
- ⚠️ **Google Analytics**: Update domain tracking

---

## 📋 Step-by-Step Implementation

### Step 1: Configure Domains in Wix Dashboard

#### 1.1 Set Primary Domain
1. Log in to Wix Dashboard
2. Go to **Settings** → **Domains & URLs**
3. Find **energievergleich.shop**
4. Click the **three-dot menu** next to it
5. Select **"Set as Primary Domain"**
6. Confirm the action

#### 1.2 Configure Domain Redirect
1. In **Domains & URLs**, look for **energievergleich.shop**
2. If not listed, click **"Connect Domain"** and add it
3. Once added, click the **three-dot menu** next to energievergleich.shop
4. Select **"Redirect to Another Domain"**
5. Choose **energievergleich.shop** as the target
6. Ensure the redirect type is **301 (Permanent Redirect)**
7. Save the configuration
8. **Note**: Redirect propagation can take up to **24 hours** to fully propagate across all DNS servers and CDNs

#### 1.3 Verify SSL/HTTPS
- Ensure both domains have valid SSL certificates
- Both should show a **green lock icon** in the browser
- Wix typically handles this automatically

---

### Step 2: Configure Google Search Console

#### 2.1 Add Both Domains as Properties
1. Go to [Google Search Console](https://search.google.com/search-console)
2. Click **"Add Property"**
3. Add **https://energievergleich.shop** (if not already added)
4. Add **https://energievergleich.shop** (if not already added)
5. Verify ownership for both domains

#### 2.2 Signal Preferred Domain via 301 Redirect + Canonical Tags
The preferred domain is signaled to Google through:
1. **301 Permanent Redirects** - Configured in Wix domain settings (see Step 1)
2. **Canonical Tags** - Automatically generated by the application pointing to `energievergleich.shop`
3. **Sitemap URLs** - All URLs in sitemap.xml reference `energievergleich.shop` only

This combination clearly signals to Google that **energievergleich.shop** is the canonical domain.

#### 2.3 Submit Sitemap
1. In the **energievergleich.shop** property:
   - Go to **Sitemaps**
   - Click **"Add/Test Sitemap"**
   - Enter: `https://energievergleich.shop/sitemap.xml`
   - Click **"Submit"**

#### 2.4 Monitor Redirect and Consolidation
1. Go to **Coverage** in the energievergleich.shop property
2. Look for redirect notices
3. Verify that Google recognizes the 301 redirect from energievergleich.shop
4. Monitor **Crawl Stats** to see when Google stops crawling energievergleich.shop
5. Check **URL Inspection** tool to verify canonical tags are correct

---

### Step 3: Configure Google Analytics

#### 3.1 Update GA4 Property Settings
1. Go to [Google Analytics](https://analytics.google.com)
2. Select your GA4 property
3. Go to **Admin** → **Data Streams**
4. Click on your web data stream
5. Verify the domain is set to **energievergleich.shop**

#### 3.2 Update Referral Exclusion List (Optional)
1. Go to **Admin** → **Data Settings** → **Referral Exclusion List**
2. Add **energievergleich.shop** to exclude it from referral traffic
3. This prevents internal redirects from being counted as referral traffic

---

### Step 4: Update Application Configuration

The following files are already configured correctly:

#### `/src/lib/seo-config.ts`
```typescript
export const SEO_CONFIG = {
  siteName: 'energievergleich.shop',
  siteUrl: 'https://energievergleich.shop',
  // ... rest of config
};
```

#### `/src/lib/seo-redirects.ts`
```typescript
export function getCanonicalUrl(pathname: string): string {
  const baseUrl = 'https://energievergleich.shop';
  return `${baseUrl}${pathname}`;
}
```

#### `/src/public/robots.txt`
```
Sitemap: https://energievergleich.shop/sitemap.xml
```

#### `/src/public/sitemap.xml`
All URLs reference `https://energievergleich.shop/`

#### `/src/components/SEOHead.tsx`
Generates canonical tags pointing to `energievergleich.shop`

---

## 🎯 Next Steps (In Wix Dashboard)

### 1. **Set Primary Domain**
   - Go to Settings → Domains & URLs
   - Find **energievergleich.shop** in your domain list
   - Click the **three-dot menu** next to it
   - Select **"Set as Primary Domain"**
   - Confirm the action

### 2. **Configure 301 Redirect for Secondary Domain**
   - In **Domains & URLs**, locate **energievergleich.shop**
   - If not listed, click **"Connect Domain"** to add it
   - Click the **three-dot menu** next to energievergleich.shop
   - Select **"Redirect to Another Domain"**
   - Choose **energievergleich.shop** as the target domain
   - Ensure redirect type is set to **301 (Permanent Redirect)**
   - Save the configuration
   - **Wait up to 24 hours** for redirect propagation

### 3. **Verify SSL/HTTPS**
   - Confirm both domains have valid SSL certificates
   - Both should display a **green lock icon** in the browser
   - Wix typically handles SSL automatically

### 4. **Google Search Console Setup**
   - Add both domains as separate properties in GSC
   - Verify ownership for both
   - Submit sitemap for energievergleich.shop only
   - Monitor Coverage and Crawl Stats for redirect recognition

### 5. **Monitor and Verify**
   - Test redirect: `curl -I https://energievergleich.shop`
   - Check canonical tags are correct
   - Verify sitemap URLs
   - Monitor GSC for consolidation progress

---

## 🔍 Verification Checklist

### Before Going Live
- [ ] Primary domain set to energievergleich.shop in Wix
- [ ] 301 redirect configured from energievergleich.shop to energievergleich.shop
- [ ] Both domains have valid SSL certificates
- [ ] robots.txt references energievergleich.shop
- [ ] sitemap.xml references energievergleich.shop
- [ ] Canonical tags point to energievergleich.shop
- [ ] Google Search Console has both properties added
- [ ] Sitemap submitted to GSC for energievergleich.shop

### Technical Verification Tests

#### Test 1: HTTP 301/308 Redirect
```bash
# Test redirect from energievergleich.shop to energievergleich.shop
curl -I https://energievergleich.shop

# Expected output:
# HTTP/1.1 301 Moved Permanently
# Location: https://energievergleich.shop/
```

#### Test 2: Canonical Tags
```bash
# Verify canonical tags on homepage
curl -s https://energievergleich.shop | grep -i "canonical"

# Expected output:
# <link rel="canonical" href="https://energievergleich.shop/">
```

#### Test 3: Sitemap URLs
```bash
# Check sitemap.xml references only energievergleich.shop
curl -s https://energievergleich.shop/sitemap.xml | grep -o "https://[^<]*"

# Expected output:
# https://energievergleich.shop/
# https://energievergleich.shop/gewerbestrom
# https://energievergleich.shop/gewerbegas
# (NO energievergleich.shop URLs)
```

#### Test 4: Robots.txt Sitemap Entry
```bash
# Verify robots.txt references correct sitemap
curl -s https://energievergleich.shop/robots.txt | grep -i sitemap

# Expected output:
# Sitemap: https://energievergleich.shop/sitemap.xml
```

#### Test 5: Redirect Chain Check
```bash
# Ensure no redirect chains (should be direct 301)
curl -I -L https://energievergleich.shop

# Expected: Single 301 redirect, no chains
```

### After Going Live
- [ ] Google Search Console shows 301 redirects recognized
- [ ] energievergleich.shop appears in search results
- [ ] energievergleich.shop traffic redirects properly
- [ ] No duplicate content warnings in GSC
- [ ] Analytics shows traffic from energievergleich.shop
- [ ] No crawl errors in GSC
- [ ] Canonical tags verified in URL Inspection tool
- [ ] Crawl Stats show energievergleich.shop crawl frequency decreasing

---

## 📊 Expected Timeline

| Phase | Timeline | Action |
|-------|----------|--------|
| **Configuration** | Day 1 | Set up Wix domains and GSC |
| **Redirect Propagation** | Up to 24 hours | DNS and CDN propagation |
| **Google Crawl** | Days 2-7 | Google discovers and crawls redirects |
| **Consolidation** | Tage bis mehrere Wochen | Domain authority consolidates (varies by crawling/indexing frequency) |
| **Cleanup** | Week 4+ | Can remove energievergleich.shop from GSC after consolidation |

---

## 🚨 Important Notes

### SEO Impact
- **Positive**: Consolidates domain authority to single domain
- **Temporary**: May see slight traffic dip during transition (1-2 weeks)
- **Recovery**: Traffic typically recovers within variable timeframe depending on Google's crawling and indexing frequency
- **Long-term**: Stronger SEO performance with single canonical domain

### Redirect Type
- **301 Permanent Redirect**: Tells search engines the move is permanent
- **Passes Authority**: 90-99% of link juice transfers to new domain
- **Updates Index**: Google will update its index to reflect the change
- **Propagation**: Can take up to 24 hours for redirect to fully propagate

### Consolidation Timeline
- **Konsolidierung kann je nach Crawling/Indexierung Tage bis mehrere Wochen dauern**
- Google's crawl frequency depends on:
  - Site authority and traffic
  - How often Google crawls your site
  - Number of backlinks pointing to old domain
  - Sitemap update frequency
- Monitor GSC regularly to track consolidation progress

### Monitoring
- Check GSC daily for the first week
- Monitor Analytics for traffic patterns
- Watch for crawl errors in GSC
- Verify redirect is working: `curl -I https://energievergleich.shop`
- Use URL Inspection tool to verify canonical tags

---

## 🔧 Troubleshooting

### Issue: Redirect Not Working
**Solution:**
1. Clear browser cache
2. Check Wix domain settings again
3. Wait 24-48 hours for DNS propagation
4. Test with: `curl -I https://energievergleich.shop`

### Issue: GSC Shows Duplicate Content
**Solution:**
1. Ensure 301 redirect is properly configured
2. Submit sitemap for energievergleich.shop only
3. Verify canonical tags are pointing to energievergleich.shop
4. Wait for Google to re-crawl (timeline varies)

### Issue: Traffic Drop After Redirect
**Solution:**
1. This is normal for 1-2 weeks
2. Verify redirect is working
3. Check GSC for crawl errors
4. Monitor Analytics for traffic recovery
5. Contact Google Search Console support if issue persists

---

## 📞 Support Resources

- **Wix Support**: https://support.wix.com/en/article/connecting-a-domain
- **Google Search Console Help**: https://support.google.com/webmasters
- **Google Analytics Help**: https://support.google.com/analytics
- **SEO Best Practices**: https://developers.google.com/search/docs

---

## ✨ Summary

Your application is **fully configured** for the domain redirect. All code-level settings are correct. You only need to:

1. ✅ Set energievergleich.shop as primary domain in Wix
2. ✅ Configure 301 redirect from energievergleich.shop in Wix
3. ✅ Update Google Search Console settings
4. ✅ Monitor the transition

The redirect will ensure that:
- All traffic from energievergleich.shop goes to energievergleich.shop
- Search engines recognize this as a permanent move
- Only energievergleich.shop appears in search results
- Domain authority consolidates to the primary domain

**Status**: Ready for Wix configuration ✅
